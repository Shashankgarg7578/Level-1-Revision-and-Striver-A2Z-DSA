...
//	Reason: The temporary array stores the non-zero elements. In the worst case, all the given array elements will be non-zero.
	...
	private static int[] moveZeros(int n, int[] arr) {
		...
		ArrayList<Integer> nzn = new ArrayList<Integer>(); 
//Make ArrayList and Traverse Array and add non-zero elements in ArrayList.
		for (int i = 0; i < n; i++) {
			if (arr[i] != 0) {
				nzn.add(arr[i]);
			}
...
//In same array place ArrayList values form 0th index.
		int nznlen = nzn.size();
		for (int i = 0; i < nznlen; i++) {
			arr[i] = nzn.get(i);
		}

//place 0’s in the end which places left which are ArrayList size to arr.length
